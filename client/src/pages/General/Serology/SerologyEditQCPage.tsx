...
import { Department, qcTypeLinkList, SerologyTypeLinkList } from "../../../utils/utils";
        ...
        <div className="button-container sticky -bottom-12 flex justify-center sm:-translate-y-12 sm:space-x-36 z-2 bg-white sm:py-6">
          <ButtonBase
            ...
            onClick={() => {
              if (selectedItem) {
                navigate({
                  pathname: `/serology/edit_qc/${selectedItem}`,
                  search: createSearchParams({
                    dep: Department.Chemistry.toString(),
                    name: SerologyTypeLinkList.find(item => item.link == selectedItem)?.name ?? ""
                  }).toString()
                })
              } else {
